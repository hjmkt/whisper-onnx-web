...
const reactLogo = "/whisper-onnx-web/assets/react-35ef61ed.svg";
const viteLogo = "/whisper-onnx-web/vite.svg";
const App$1 = "";
const positionalEmbeddingBaseUrl = "/whisper-onnx-web/assets/positional_embedding_base.bin-f2fc9c24.compressed";
const positionalEmbeddingSmallUrl = "/whisper-onnx-web/assets/positional_embedding_small.bin-10ab3821.compressed";
const processorCode = 'class WhisperProcessor extends AudioWorkletProcessor {\n    // Custom AudioParams can be defined with this static getter.\n    static get parameterDescriptors() {\n        return [{ name: "hoge", defaultValue: 1 }];\n    }\n\n    constructor(options) {\n        super();\n        this.buffer = [];\n        this.chunkLength = options.processorOptions.chunkLength;\n    }\n\n    process(inputs, outputs, parameters) {\n        let input = inputs[0];\n        //let output = outputs[0];\n        let gain = parameters.hoge;\n        for (let i = 0; i < input[0].length; i++) {\n            this.buffer.push(input[0][i]);\n        }\n        if (this.buffer.length >= this.chunkLength * 16000) {\n            this.port.postMessage(\n                this.buffer.slice(0, this.chunkLength * 16000)\n            );\n            this.buffer = this.buffer.slice(this.chunkLength * 8000);\n        }\n        return true;\n    }\n}\n\nregisterProcessor("whisper-processor", WhisperProcessor);\n';
const preprocessorBaseModelUrl = "/whisper-onnx-web/assets/preprocessor_base_5s_int8.onnx-ea26ddaf.compressed";
const preprocessorSmallModelUrl = "/whisper-onnx-web/assets/preprocessor_small_10s_int8.onnx-08e67b6f.compressed";
const encoderBaseModelUrl = "/whisper-onnx-web/assets/encoder_base_5s_int8.onnx-56787330.compressed";
const encoderSmallModelUrl = "/whisper-onnx-web/assets/encoder_small_10s_int8.onnx-15c2dbcb.compressed";
const decoderBaseModelUrl = "/whisper-onnx-web/assets/decoder_base_5s_int8.onnx-d233fd9f.compressed";
const decoderSmallModelUrl0 = "/whisper-onnx-web/assets/decoder_small_10s_int8.onnx.0-236713b7.compressed";
const decoderSmallModelUrl1 = "/whisper-onnx-web/assets/decoder_small_10s_int8.onnx.1-a5417d79.compressed";
function WorkerWrapper() {
  return new Worker("/whisper-onnx-web/assets/whisper_worker-7397964e.js");
}
var dist = {};
var circle = {};
...
var Circle$1 = function(_super) {
  ...
  Object.defineProperty(Circle2.prototype, "text", {
    get: function() {
      ...
      return React.createElement(
        "text",
        { style: textStyle, fill: textColor, x: "50%", y: "50%", dx: "-25", textAnchor: "middle" },
        progress,
  ...
  Circle2.prototype.render = function() {
    ...
    return React.createElement(
      "svg",
      { width: svgSize, height: svgSize, viewBox: "-25 -25 400 400" },
      React.createElement("circle", { stroke: bgColor, cx: "175", cy: "175", r: "175", strokeWidth: lineWidth, fill: "none" }),
      React.createElement("circle", { stroke: progressColor, transform: "rotate(-90 175 175)", cx: "175", cy: "175", r: "175", strokeDasharray: "1100", strokeWidth: lineWidth, strokeDashoffset: "1100", strokeLinecap, fill: "none", style: { strokeDashoffset, transition }, onTransitionEnd: onAnimationEnd }),
      text
    );
  };
...
const Circle = /* @__PURE__ */ getDefaultExportFromCjs(dist);
const CHUNK_LENGTH = 10;
let whisper_workers = {
  base: {
    0: { running: false, worker: new WorkerWrapper() },
    1: { running: false, worker: new WorkerWrapper() }
  },
  small: {
    0: { running: false, worker: new WorkerWrapper() },
    1: { running: false, worker: new WorkerWrapper() }
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      db2.createObjectStore("files", { keyPath: "name" });
    };
  });
}
async function cache(name, buffer) {
  const db2 = await openDatabase();
  const transaction = db2.transaction("files", "readwrite");
  const store = transaction.objectStore("files");
  store.put({ name, buffer });
}
async function getCachedData(name) {
  const db2 = await openDatabase();
  const transaction = db2.transaction("files", "readonly");
  const store = transaction.objectStore("files");
...
async function initWhisper(progressCallback, textCallback = () => {
  ...
  if (!whisperInitialized) {
    Promise.all([
      ...
      if (cachedData.every((v2) => v2 !== null)) {
        let v2 = cachedData;
        for (let i = 0; i < 2; i++) {
          whisper_workers["base"][i].worker.postMessage({
            type: "init",
            modelType: "base",
            positionalEmbedding: v2[0],
            preprocessorModel: v2[1],
            encoderModel: v2[2],
            decoderModel: v2[3],
            chunkLength: 5,
            debug: false,
        ...
        for (let i = 0; i < 2; i++) {
          whisper_workers["small"][i].worker.postMessage({
            type: "init",
            modelType: "small",
            positionalEmbedding: v2[4],
            preprocessorModel: v2[5],
            encoderModel: v2[6],
            decoderModel: v2[7],
            chunkLength: 10,
            debug: false,
      ...
      Promise.all([
        ...
        const lengths = fetchRes.map(
          (v2) => Number(v2.headers.get("content-length"))
        );
        const total = lengths.reduce((a, b) => a + b, 0);
        let buffers = lengths.map((v2) => new Uint8Array(v2));
        let readers = fetchRes.map((v2) => v2.body.getReader());
        let chunks = [0, 0, 0, 0, 0, 0, 0, 0, 0];
        let chunk = 0;
        Promise.all(readers.map((v2) => v2.read())).then(
          function process(res) {
            if (res.every((v2) => v2.done)) {
              Promise.all(
                buffers.map(
                  (v2) => new Response(
                    new Blob([v2]).stream().pipeThrough(
                ...
                let n_bytes = v2[7].byteLength + v2[8].byteLength;
                let buffer = new ArrayBuffer(n_bytes);
                buffer = new Uint8Array(buffer);
                buffer.set(new Uint8Array(v2[7]), 0);
                buffer.set(
                  new Uint8Array(v2[8]),
                  v2[7].byteLength
                );
                ...
                cache("embedding-base", v2[0]);
                cache("preproc-base-5s", v2[1]);
                cache("encoder-base-5s", v2[2]);
                cache("decoder-base-5s", v2[3]);
                cache("embedding-small", v2[4]);
                cache("preproc-small-10s", v2[5]);
                cache("encoder-small-10s", v2[6]);
            ...
            for (let i = 0; i < res.length; i++) {
              if (!res[i].done) {
                if (buffers[i].byteLength < chunks[i] + res[i].value.byteLength) {
    ...
    whisper_workers["base"][i].worker.onmessage = (e) => {
      if (e.data.action == "result") {
        ...
        if (tmpSmallText.length >= headText.length) {
          ...
          headText = tmpSmallText + headText.slice(tmpSmallText.length);
        }
        let text = fixedText + headText + tailText;
        ((p2) => textCallback(p2))(text);
        whisper_workers["base"][i].running = false;
      } else {
        let tmpTexts = [];
        for (let i2 = 0; i2 < lastBaseTexts.length; i2++) {
          tmpTexts.push(lastBaseTexts[i2]);
        }
...
function runWhisper(model_type, tensor, textCallback, tokenCallback = () => {
}) {
...
async function transcribe() {
  ...
  const tmpCtx = new AudioContext({ sampleRate: 16e3 });
...
const App = ({ isProduction }) => {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "App", children: [
    /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx("a", { href: "https://vitejs.dev", target: "_blank", rel: "noreferrer", children: /* @__PURE__ */ jsx("img", { src: viteLogo, className: "logo", alt: "Vite logo" }) }),
      /* @__PURE__ */ jsx("a", { href: "https://reactjs.org", target: "_blank", rel: "noreferrer", children: /* @__PURE__ */ jsx(
        "img",
        {
          src: reactLogo,
    ...
    /* @__PURE__ */ jsx("h1", { children: "Whisper ONNX Web" }),
    /* @__PURE__ */ jsx("div", { children: /* @__PURE__ */ jsx(Circle, { progress }) }),
    /* @__PURE__ */ jsx(
      "textarea",
      {
        readOnly: true,
...
client.createRoot(document.getElementById("root")).render(
  /* @__PURE__ */ jsx(React$1.StrictMode, { children: /* @__PURE__ */ jsx(App, { isProduction: true }) })
);
//# sourceMappingURL=index-999c58bb.js.map
